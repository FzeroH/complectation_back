...
module.exports.login = async function (req, res){
    ...
    try {
        ...
        if (user.role_name === 'teacher'){
            data = await db.one(`SELECT sd.users_id, role_name, u.users_first_name as first_name, u.users_last_name as last_name, users_email,
            ...
            data = await db.one(`SELECT users_id, users_first_name as first_name,
                                              users_last_name as last_name, users_email, users_password, role_name
                                       FROM users JOIN role ON users.role_id = role.role_id WHERE users_id = ${user.users_id};`,)
        }
        res.status(200).json({
