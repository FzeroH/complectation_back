...
module.exports.getPublications = async function (req, res) {
  try {
      const result = await db.many(`
        ...
        FROM publication JOIN company ON publication.company_id = company.company_id ORDER BY publication_id;`)
...
module.exports.getFilteredPublications = async function (req, res) {
    const { direction, field } = req.query
    try {
        const result = await db.many(` 
                SELECT publication_id as id, publication_author, publication_title, publication_year, publication_cost, company_name
                FROM publication JOIN company ON publication.company_id = company.company_id;
                ORDER BY publication_year $1;`, [direction, field])
        return res.status(200).json(result)
    } catch (e) {
        console.error(e)
        res.status(500).json({
